...
import {
  ...
  Target, Calendar, Star, Users, Zap, Activity,
} from "lucide-react";
import {
...
export default function RealAnalyticsTab({ data }: Props) {
  const { entResults, attendance, homework, aiInsights, hero, entForecast } = data;
  ...
  const radarData = latestEnt?.subjects ? latestEnt.subjects.map((s: any) => ({
    subject: s?.name?.split(" ")?.[0] || '', // first word only for labels
    fullName: s?.name || '',
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="pt-1 border-t mt-2">
              <div className="flex justify-between text-xs font-medium">
                ...
                <span className="text-emerald-600 font-bold">
                  {hero?.entProgressPct || 0}%
                </span>
              </div>
              <Progress value={hero?.entProgressPct || 0} className="h-2 mt-1" />
              <div className="text-[10px] text-muted-foreground mt-0.5">
                {hero?.entLastScore || 0} из {hero?.entTarget || 0} баллов (Δ+{hero?.entDelta || 0})
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {aiInsights?.length > 0 ? aiInsights.map((ins: any, i: number) => (
              <div key={i} className={cn(
                "p-2.5 rounded-lg border text-xs",
                ins.type === "warning" ? "bg-amber-50 border-amber-200 dark:bg-amber-950/20 dark:border-amber-800" :
                ins.type === "success" ? "bg-emerald-50 border-emerald-200 dark:bg-emerald-950/20 dark:border-emerald-800" :
                "bg-blue-50 border-blue-200 dark:bg-blue-950/20 dark:border-blue-800"
              )}>
                <div className="flex items-center gap-1.5 mb-1">
                  <Badge variant="outline" className={cn(
                    "text-[9px] px-1.5 py-0",
                    ins.priority === "high" ? "bg-red-100 text-red-700 border-red-200" :
                    ins.priority === "medium" ? "bg-amber-100 text-amber-700 border-amber-200" :
                    "bg-slate-100 text-slate-600 border-slate-200"
                  )}>
                    {ins.priority === "high" ? "Важно" : ins.priority === "medium" ? "Средний" : "Инфо"}
                  </Badge>
                  <span className="font-semibold">{ins.title}</span>
                </div>
                <p className="text-muted-foreground leading-snug">{ins.body}</p>
              </div>
            )) : (
              <div className="text-sm text-muted-foreground italic">Нет доступных инсайтов</div>
            )}
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={entTrendData}>
              ...
              <ReferenceLine y={hero?.entTarget || 0} stroke="#ef4444" strokeDasharray="4 4" label={{ value: `Цель: ${hero?.entTarget || 0}`, position: "insideTopRight", fontSize: 10, fill: "#ef4444" }} />
